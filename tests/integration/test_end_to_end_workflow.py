...
class TestEndToEndWorkflow:
    ...
    def temp_dataset_dir(self):
        ...
        folders = ["files/code", "files/data", "files/documents", 
                  "files/images", "files/presentations"]
    ...
    def folder_data_items_csv(self, temp_dataset_dir):
        ...
        data = [
            ["file", "status", "type", "title", "alternative", "author", "contributor", 
             "date", "license", "description", "keywords", "language", "temporal", 
             "spatial", "accessRights", "identifier", "rights"],
            ["files/code/", "pending", "http://purl.org/coar/resource_type/c_5ce6",
             "fr:Fichiers de code|en:Code Files", "", "Doe,John", "", "2024-01-01", 
             "CC-BY-4.0", "fr:Scripts|en:Scripts", "fr:code|en:code", "fr", 
             "2024", "Global", "Open Access", "", "test-group,ROLE_READER"],
            ["files/data/", "pending", "http://purl.org/coar/resource_type/c_ddb1",
             "fr:Données|en:Data", "", "Doe,John", "", "2024-01-01", 
             "CC-BY-4.0", "fr:Données test|en:Test data", "fr:données|en:data", "fr", 
             "2024", "Global", "Open Access", "", "test-group,ROLE_READER"]
        ]
    ...
    def folder_collections_csv(self, temp_dataset_dir):
        ...
        data = [
            ["title", "status", "description", "keywords", "language", "creator", 
             "contributor", "publisher", "date", "rights", "coverage", "relation", 
             "source", "data_items"],
            ["fr:Collection Test|en:Test Collection", "private", 
             "fr:Collection de test|en:Test collection", "fr:test|en:test", 
             "fr", "Doe,John", "", "Test Org", "2024-01-01", "CC-BY-4.0", 
             "Global", "Test relation", "Test source", "files/code/|files/data/"]
        ]
    ...
    def test_upload_workflow(self, test_config, temp_dataset_dir, 
                           folder_data_items_csv, mock_api_responses):
        ...
        with patch('requests.Session') as mock_session:
            ...
            client.validate_dataset(
                mode="folder",
                folder_config=folder_data_items_csv
            )
    ...
    def test_collection_creation_workflow(self, test_config, temp_dataset_dir,
                                        folder_collections_csv, mock_api_responses):
        ...
        upload_data = [
            ...
            ["10.34847/nkl.test123", "test.py,hash123", "Code Files", "OK", 
             json.dumps(mock_api_responses['upload_response'])],
            ["10.34847/nkl.test456", "data.csv,hash456", "Data Files", "OK",
             json.dumps(mock_api_responses['upload_response'])]
        ]
        ...
        with patch('requests.Session') as mock_session:
            ...
            assert 'fr:Collection Test|en:Test Collection' in collections_data[0]['title']
    ...
    def test_curator_modification_workflow(self, test_config, mock_api_responses):
        ...
        try:
            with patch('requests.get') as mock_get, patch('requests.put') as mock_put:
                ...
                mock_get_response.json.return_value = mock_api_responses['data_item_response']
    ...
    def test_complete_workflow_integration(self, test_config, temp_dataset_dir,
                                         folder_data_items_csv, folder_collections_csv,
                                         mock_api_responses):
        ...
        with patch('requests.Session') as mock_session:
            ...
            collections_data = collection_client._load_folder_collections(folder_collections_csv)
            assert len(collections_data) == 1
    ...
    def test_workflow_output_files(self, test_config, temp_dataset_dir,
                                  folder_data_items_csv, mock_api_responses):
