...
class TestBatchUploadOperations:
    ...
    def sample_csv_dataset(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open(csv_path, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow([
                    "file", "status", "type", "title", "author", "date", 
                    "license", "description", "keywords", "language"
                ])
    ...
    def test_batch_upload_partial_success(self, mock_session, batch_config, sample_csv_dataset):
        ...
        responses = [
            ...
            MagicMock(status_code=201, json=lambda: {"id": "data1", "status": "pending"}),
            ...
            MagicMock(status_code=201, json=lambda: {"id": "data3", "status": "pending"}),
            
            # file4.txt - validation failure
            MagicMock(status_code=200, json=lambda: {"sha1": "ghi789"}),
            MagicMock(status_code=422, json=lambda: {"error": "Validation failed", "details": {"title": ["Required"]}})
        ]
    ...
    @patch('requests.Session')
    def test_batch_upload_retry_behavior(self, mock_session, batch_config, sample_csv_dataset):
        ...
        upload_responses = [
            ...
            MagicMock(status_code=201, json=lambda: {"id": "retry_data", "status": "pending"})
        ]
    ...
    def test_batch_upload_error_aggregation(self, batch_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open(csv_path, 'w', newline='', encoding='utf-8') as f:
                ...
                writer.writerow(["nonexistent.txt", "pending", "invalid-type", ""])  # Missing file, invalid type, empty title
                writer.writerow(["", "pending", "http://purl.org/coar/resource_type/c_ddb1", "Valid"])  # Empty file
                writer.writerow(["../outside.txt", "pending", "http://purl.org/coar/resource_type/c_ddb1", "Outside"])  # Outside base path
...
class TestCollectionBatchOperations:
    ...
    def folder_collections_config(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as f:
            writer = csv.writer(f)
            writer.writerow([
                "title", "description", "keywords", "language", "creator", 
                "publisher", "date", "rights", "data_items"
            ])
            
            # Multiple collections with different data items
            collections_data = [
                ["Collection A", "First test collection", "test;batch", "en", 
                 "Test Creator", "Test Pub", "2024-01-01", "CC-BY-4.0", 
                 "10.34847/nkl.test000|10.34847/nkl.test001"],
                ["Collection B", "Second test collection", "test;batch", "en", 
                 "Test Creator", "Test Pub", "2024-01-01", "CC-BY-4.0", 
                 "10.34847/nkl.test002|10.34847/nkl.test003"],
                ["Collection C", "Third test collection", "test;batch", "en", 
                 "Test Creator", "Test Pub", "2024-01-01", "CC-BY-4.0", 
    ...
    @patch('requests.Session')
    def test_batch_collection_creation_partial_failure(self, mock_session, collection_batch_config,
        ...
        responses = [
            ...
            MagicMock(status_code=422, json=lambda: {"error": "Validation failed", "details": {"title": ["Invalid"]}}),
            
            # Collection B - success
            MagicMock(status_code=201, json=lambda: {"id": "collection_b", "title": "Collection B"}),
            
            # Collection C - success
            MagicMock(status_code=201, json=lambda: {"id": "collection_c", "title": "Collection C"})
        ]
    ...
    @patch('requests.Session')
    def test_collection_data_validation_batch(self, mock_session, collection_batch_config,
                                            mock_upload_output, folder_collections_config):
        ...
        try:
            ...
            assert "validation" in str(e).lower() or isinstance(e, (NakalaValidationError, ValueError))
    ...
    def test_concurrent_validation_operations(self, concurrent_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for dataset_path in datasets:
                ...
                try:
                    upload_client.validate_dataset(mode="csv", dataset_path=dataset_path)
