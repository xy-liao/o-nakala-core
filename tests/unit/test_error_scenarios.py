...
class TestNetworkAndAPIErrors:
    ...
    def test_api_timeout_error(self, mock_session, mock_config):
        ...
        import requests
        mock_session.return_value.post.side_effect = requests.exceptions.Timeout("Request timed out")
    ...
    def test_api_http_error_responses(self, mock_session, mock_config):
        ...
        for error_code in error_codes:
            ...
            try:
                ...
                result = upload_client.upload_single_file(test_file, {"title": "Test", "type": "test"})
    ...
    def test_metadata_missing_required_fields(self, validation_config):
        ...
        for metadata in incomplete_metadata_sets:
            try:
                ...
                if prepared:
                    title_found = any('title' in m.get('propertyUri', '') for m in prepared)
                    type_found = any('type' in m.get('propertyUri', '') for m in prepared)
...
class TestConcurrencyAndResourceErrors:
    ...
    def test_large_file_memory_handling(self, resource_config):
        ...
        try:
            ...
            try:
                sha1_hash = upload_client.file_processor._calculate_file_hash(large_file)
