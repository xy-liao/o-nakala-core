...
class TestNakalaCommonUtils:
    ...
    def test_prepare_nakala_metadata_basic(self, utils):
        ...
        title_meta = next((m for m in result if 'title' in m.get('propertyUri', '')), None)
...
class TestNakalaPathResolver:
    ...
    def test_resolver_initialization(self, resolver, mock_config):
        ...
        # Path.resolve() might resolve symlinks, so check that base_path is contained
        assert mock_config.base_path in str(resolver.base_path) or str(resolver.base_path) in mock_config.base_path
