...
class ContentSimilarityAnalyzer:
    ...
    def calculate_metadata_similarity(self, meta1: Dict[str, Any], meta2: Dict[str, Any]) -> float:
...
class RelationshipTypeClassifier:
    ...
    def __init__(self):
        self.relationship_patterns = {
            'isPartOf': [
                'part of', 'belongs to', 'contained in', 'subset of',
                'volume', 'chapter', 'section', 'episode'
            ],
            'hasPart': [
                'contains', 'includes', 'has part', 'comprises',
                'collection of', 'series of', 'set of'
            ],
            'isVersionOf': [
                'version', 'edition', 'revision', 'update',
                'draft', 'final', 'revised'
            ],
            'isFormatOf': [
                'format', 'copy', 'reproduction', 'digitization',
                'transcription', 'translation'
            ],
            'references': [
                'cites', 'references', 'mentions', 'discusses',
                'based on', 'derived from', 'inspired by'
            ],
            'isReferencedBy': [
                'cited by', 'referenced by', 'mentioned in',
                'discussed in', 'basis for'
            ],
            'requires': [
                'requires', 'depends on', 'needs', 'prerequisite',
                'supplemented by', 'accompanied by'
            ],
            'isRequiredBy': [
                'required by', 'dependency of', 'needed by',
                'supplement to', 'accompanies'
            ],
            'replaces': [
                'replaces', 'supersedes', 'updates', 'corrects',
                'new version of', 'improvement of'
            ],
            'isReplacedBy': [
                'replaced by', 'superseded by', 'updated by',
                'corrected by', 'improved by'
            ],
            'conformsTo': [
                'conforms to', 'follows', 'implements', 'based on standard',
                'according to', 'compliant with'
            ],
            'relation': [
                'related to', 'associated with', 'connected to',
                'similar to', 'comparable to', 'linked to'
            ]
        }
    ...
    def classify_relationship(
        ...
        inferred_type = self._infer_relationship_from_metadata(source_metadata, target_metadata)
        if inferred_type:
        ...
        if similarity_score > 0.6:
            return "relation", similarity_score, "Generic relationship based on content similarity"
    ...
    def _detect_explicit_relationship(self, source_text: str, target_text: str) -> Optional[str]:
    ...
    def _infer_relationship_from_metadata(
        ...
        if any(indicator in source_title for indicator in version_indicators) or \
           any(indicator in target_title for indicator in version_indicators):
            return 'isVersionOf'
...
class RelationshipDiscoveryService:
    ...
    async def discover_relationships(
        ...
        try:
            ...
            logger.warning(f"Could not get user profile for relationship discovery: {e}")
            all_resources = []
        
        # Filter out the source resource itself
        if source_id:
            all_resources = [r for r in all_resources if r.get('identifier') != source_id]
        ...
        if not all_resources:
            analysis_notes.append("No user resources available for relationship discovery")
        else:
            analysis_notes.append(f"Analyzing relationships with {len(all_resources)} user resources")
            ...
            for resource in all_resources:
                ...
                similarity = await self._calculate_resource_similarity(source_metadata, target_metadata)
                similarity_matrix[target_id] = similarity
                
                # Only suggest if similarity is above threshold
                if similarity >= min_confidence:
                    # Classify relationship type
                    rel_type, confidence, reason = self.relationship_classifier.classify_relationship(
                    ...
                    suggestion = RelationshipSuggestion(
                        target_id=target_id,
                        target_title=self._extract_meta_value(resource.get('metas', []), 'title'),
    ...
    async def _calculate_resource_similarity(
        ...
        text_similarity = self.similarity_analyzer.calculate_text_similarity(source_text, target_text)
    ...
    def generate_relationship_report(self, analysis: RelationshipAnalysis) -> str:
        ...
        if analysis.suggestions:
            avg_confidence = sum(s.confidence for s in analysis.suggestions) / len(analysis.suggestions)
...
# Factory function
def create_relationship_discovery_service(user_client: NakalaUserInfoClient) -> RelationshipDiscoveryService:
