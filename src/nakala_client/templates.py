...
class TemplateGenerator:
    ...
    async def generate_template(
        ...
        if include_optional:
            all_fields.extend([f for f in recommended_fields if f not in required_fields])
        
        # Generate template fields
        template_fields = []
        for field_name in all_fields:
            field = await self._generate_template_field(field_name, resource_type, user_context)
            if field:
    ...
    def _get_recommended_fields(self, resource_type: str, user_context: Dict[str, Any] = None) -> List[str]:
        """Get recommended fields based on resource type and context."""
        recommended = ['keywords', 'contributor', 'alternative', 'identifier', 
                      'temporal', 'spatial', 'rights', 'accessRights']
    ...
    async def _generate_template_field(
        ...
        # Generate examples
        examples = await self._generate_examples(field_name, controlled_vocab, user_context)
    ...
    async def _generate_examples(
        ...
        if not examples:
            ...
            examples = contextual_examples.get(field_name, [f"Example {field_name} value"])
    ...
    def _get_context_examples(self, field_name: str, user_context: Dict[str, Any]) -> List[str]:
    ...
    def _generate_help_text(self, field_name: str, data_type: str, controlled_vocab: Optional[str]) -> str:
        ...
        if controlled_vocab:
            help_text += f' This field uses controlled vocabulary from {controlled_vocab}.'
    ...
    def _get_default_value(self, field_name: str, user_context: Dict[str, Any] = None) -> Optional[str]:
    ...
    def _generate_template_description(self, resource_type: str) -> str:
        ...
        return descriptions.get(resource_type, f'Metadata template for {resource_type} resources.')
...
# Factory function
def create_template_generator(vocabulary_service: NakalaVocabularyService) -> TemplateGenerator:
