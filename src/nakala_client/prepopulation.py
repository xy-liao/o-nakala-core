...
class UserContextService:
    ...
    async def build_user_context(self, api_key: str) -> UserContext:
        ...
        try:
            ...
            logger.info(f"Built user context with {len(context.common_keywords)} keywords, "
    ...
    def _detect_default_language(self, user_profile: Dict[str, Any]) -> str:
        ...
        all_items = user_profile.get('datasets', []) + user_profile.get('collections', [])
    ...
    async def _analyze_datasets(self, context: UserContext, datasets: List[Dict[str, Any]]):
        ...
        for dataset in datasets[-10:]:  # Analyze last 10 datasets
            ...
            for meta in metas:
                if 'license' in meta.get('propertyUri', '').lower():
                    ...
                    if license_value and license_value not in context.preferred_licenses:
                        context.preferred_licenses.append(license_value)
            
            # Extract creators/contributors for collaboration patterns
            for meta in metas:
                if 'creator' in meta.get('propertyUri', '').lower() or 'contributor' in meta.get('propertyUri', '').lower():
        ...
        context.preferred_licenses = self._get_most_common(context.preferred_licenses, 5)
        context.frequent_collaborators = self._get_most_common(context.frequent_collaborators, 10)
    
    async def _analyze_collections(self, context: UserContext, collections: List[Dict[str, Any]]):
    ...
    def _detect_geographic_focus(self, user_profile: Dict[str, Any]) -> Optional[str]:
        ...
        for item in all_items:
            metas = item.get('metas', [])
            for meta in metas:
                if 'coverage' in meta.get('propertyUri', '').lower() or 'spatial' in meta.get('propertyUri', '').lower():
                    value = meta.get('value', '')
                    if value:
    ...
    def _detect_historical_period(self, user_profile: Dict[str, Any]) -> Optional[str]:
        ...
        for item in all_items:
            metas = item.get('metas', [])
            for meta in metas:
                if 'temporal' in meta.get('propertyUri', '').lower() or 'date' in meta.get('propertyUri', '').lower():
                    value = meta.get('value', '')
                    if value:
    ...
    def _extract_meta_value(self, metas: List[Dict], property_name: str, language: str = "en") -> str:
...
class FileMetadataExtractor:
    ...
    async def extract_file_metadata(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            if file_path.exists():
                ...
                metadata['suggested_type'] = self._suggest_resource_type(metadata['mime_type'])
            
            # Generate suggestions based on filename
            metadata['suggested_title'] = self._generate_title_from_filename(file_path.name)
            metadata['suggested_description'] = self._generate_description_from_filename(file_path.name)
            metadata['suggested_keywords'] = self._extract_keywords_from_filename(file_path.name)
            
            # Extract technical metadata based on file type
            if metadata['mime_type']:
                metadata['technical_metadata'] = await self._extract_technical_metadata(file_path, metadata['mime_type'])
    ...
    async def _extract_technical_metadata(self, file_path: Path, mime_type: str) -> Dict[str, Any]:
        """Extract technical metadata based on file type."""
        technical = {}
...
class PrePopulationAssistant:
    ...
    def __init__(self, user_client: NakalaUserInfoClient, vocab_service: NakalaVocabularyService):
        self.user_client = user_client
    ...
    async def pre_populate_template(
        ...
        logger.info(f"Pre-populated {len(populated_fields)} fields in {processing_time:.2f}s")
        return result
    
    async def _populate_field(self, field: TemplateField, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _populate_title_field(self, field: TemplateField, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _populate_description_field(self, field: TemplateField, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _populate_creator_field(self, field: TemplateField, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _populate_date_field(self, field: TemplateField, context: Dict[str, Any]) -> Dict[str, Any]:
        """Populate date field."""
    ...
    async def _populate_language_field(self, field: TemplateField, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _populate_keywords_field(self, field: TemplateField, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _populate_license_field(self, field: TemplateField, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _populate_type_field(self, field: TemplateField, context: Dict[str, Any]) -> Dict[str, Any]:
        """Populate type field."""
        file_context = context.get('file', {})
    ...
    async def _populate_contributor_field(self, field: TemplateField, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if collaborators:
            return {
                'value': collaborators[0] if len(collaborators) == 1 else ';'.join(collaborators[:3]),
                'confidence': 0.6,
    ...
    async def _populate_spatial_field(self, field: TemplateField, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _populate_temporal_field(self, field: TemplateField, context: Dict[str, Any]) -> Dict[str, Any]:
...
def create_prepopulation_assistant(
    ...
    vocab_service: NakalaVocabularyService
