...
class NakalaVocabularyCache:
    ...
    def __init__(self, cache_dir: str = None, cache_ttl_hours: int = 24):
        self.cache_dir = Path(cache_dir) if cache_dir else Path.home() / ".nakala" / "cache"
...
class NakalaVocabularyService:
    ...
    async def discover_vocabularies(self, force_refresh: bool = False) -> Dict[str, List[VocabularyEntry]]:
        ...
        for vocab_name, endpoint in vocabulary_endpoints.items():
            ...
            try:
                ...
                vocabulary_entries = self._parse_vocabulary_response(vocab_data, vocab_name)
                ...
                logger.info(f"Discovered {len(vocabulary_entries)} entries for {vocab_name}")
    ...
    def _parse_vocabulary_response(self, response_data: Dict[str, Any], vocab_name: str) -> List[VocabularyEntry]:
    ...
    def search_vocabulary(self, vocab_name: str, query: str, limit: int = 10) -> List[VocabularyEntry]:
        ...
        for entry in vocabulary:
            if (query_lower in entry.label.lower() or 
                ...
                (entry.description and query_lower in entry.description.lower())):
    ...
    def get_vocabulary_suggestions(self, vocab_name: str, partial_value: str, limit: int = 5) -> List[VocabularyEntry]:
        ...
        for entry in vocabulary:
            if (entry not in suggestions and 
                entry.label.lower().startswith(partial_lower)):
                suggestions.append(entry)
                
        # Finally fuzzy matches
        for entry in vocabulary:
            if (entry not in suggestions and 
                (partial_lower in entry.value.lower() or 
                 partial_lower in entry.label.lower())):
                suggestions.append(entry)
...
class MetadataSchemaGenerator:
    ...
    def generate_field_schema(self, property_uri: str, field_config: Dict[str, Any] = None) -> FieldSchema:
    ...
    def _determine_data_type(self, property_uri: str, field_config: Dict[str, Any]) -> str:
    ...
    def _generate_examples(self, property_uri: str, controlled_vocab: Optional[str]) -> List[str]:
        ...
        if controlled_vocab:
            ...
            if 'title' in property_uri.lower():
                examples = ["Research Dataset Title", "fr:Titre en français|en:English Title"]
            elif 'description' in property_uri.lower():
                examples = ["Detailed description of the resource", "fr:Description|en:Description"]
    ...
    def _generate_help_text(self, property_uri: str, field_config: Dict[str, Any]) -> str:
        ...
        help_texts = {
            'title': 'The main title of the resource. For multilingual titles, use format: "fr:French title|en:English title"',
            'description': 'A detailed description of the resource content and purpose.',
            'creator': 'Primary authors/creators in format "Surname,Givenname". Separate multiple creators with semicolons.',
            ...
            'type': 'Resource type from COAR vocabulary (e.g., http://purl.org/coar/resource_type/c_ddb1).',
            ...
            'keywords': 'Keywords describing the resource. For multilingual: "fr:mot1;mot2|en:word1;word2"',
...
def create_vocabulary_service(config: NakalaConfig, cache_ttl_hours: int = 24) -> NakalaVocabularyService:
