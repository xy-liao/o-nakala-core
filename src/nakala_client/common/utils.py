...
class NakalaCommonUtils:
    ...
    FOLDER_TYPE_MAPPINGS = {
        ...
        'documents': ['documents', 'documentation', 'research documents', 'papers', 'docs'],
        'images': ['images', 'collection d\'images', 'photos', 'visualizations', 'img', 'pictures'],
        'presentations': ['presentations', 'matériaux de présentation', 'slides', 'diapositives', 'ppt'],
    ...
    def parse_multilingual_field(value: str) -> List[Tuple[Optional[str], str]]:
        ...
        for part in value.split('|'):
            part = part.strip()
            if ':' in part and len(part.split(':', 1)[0]) <= 3:  # Language codes are typically 2-3 chars
    ...
    @staticmethod
    def prepare_nakala_metadata(config: Dict[str, str], 
        ...
        for field_name, property_uri in field_mapping.items():
            ...
            parsed_values = NakalaCommonUtils.parse_multilingual_field(config[field_name])
    ...
    @staticmethod
    def prepare_rights_list(rights_string: str, 
                           valid_group_ids: Optional[List[str]] = None) -> List[Dict[str, str]]:
        """
    ...
    def detect_file_type(file_path: str) -> str:
        ...
        extension_mapping = {
            'code': ['.py', '.js', '.java', '.cpp', '.h', '.cs', '.php', '.r', '.sql', '.sh'],
...
class NakalaPathResolver:
    ...
    def list_files(self, pattern: str = "*") -> List[str]:
        ...
        return [str(p.relative_to(self.base_path)) for p in self.base_path.glob(pattern) if p.is_file()]


def setup_common_logging(log_file: str = 'nakala_client.log', 
