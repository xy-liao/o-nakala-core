    ...
    def process_folder_structure(self, folder_config_path: str) -> List[Dict[str, Any]]:
        ...
        for root, _, files in os.walk(self.config.base_path):
            for file in files:
                ...
                try:
                    rel_path = self.path_resolver.get_relative_path(os.path.dirname(file_path))
...
class NakalaUploadClient:
    ...
    def create_dataset(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                logger.warning("Rate limited by API, will retry with exponential backoff")
                raise NakalaAPIError(
                    "Rate limited by API",
                    status_code=response.status_code,
                    response_text=response.text
                ...
                # Server errors - retry
                logger.warning(f"Server error (HTTP {response.status_code}), will retry")
                raise NakalaAPIError(
    ...
    def _process_folder_entry(self, result: Dict[str, Any], output_writer) -> None:
        ...
        try:
            ...
            # Write output
            file_list = ','.join([f"{os.path.basename(f)},{fi['sha1']}" 
    ...
    def validate_dataset(self, mode: str = None, dataset_path: str = None, folder_config: str = None) -> None:
    ...
    def _validate_csv_dataset(self, csv_path: str) -> None:
        ...
        for num, data in enumerate(dataset, 1):
            logger.info(f"Validating entry {num}/{total_entries}: {data[3] if len(data) > 3 else 'unknown'}")
        ...
        logger.info(f"Validation complete: {valid_entries}/{total_entries} entries valid")
    ...
    def _validate_folder_dataset(self, folder_config_path: str) -> None:
        ...
        logger.info(f"Validation complete: {valid_folders}/{total_folders} folders valid")

    def run(self, mode: str = None, dataset_path: str = None, folder_config: str = None) -> None:
...
def create_upload_client(api_url: str = None, api_key: str = None, **kwargs) -> NakalaUploadClient:
    """
...
def main():
    ...
    parser.add_argument('--api-key', required=True,
                      help='Nakala API key')
    parser.add_argument('--dataset', default='sample_dataset/folder_data_items.csv',
                      help='Path to dataset CSV file or folder')
    parser.add_argument('--base-path', default='sample_dataset',
                      help='Base directory for files')
    parser.add_argument('--mode', choices=['csv', 'folder'], default='folder',
