...
class SAMLProvider(SSOProvider):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.default_attribute_map = {
            'user_id': ['urn:oid:0.9.2342.19200300.100.1.1', 'uid', 'eduPersonPrincipalName'],
            'email': ['urn:oid:0.9.2342.19200300.100.1.3', 'mail', 'email'],
            'name': ['urn:oid:2.5.4.3', 'cn', 'displayName'],
            'institution': ['urn:oid:1.3.6.1.4.1.5923.1.1.1.3', 'schacHomeOrganization', 'o'],
            'roles': ['urn:oid:1.3.6.1.4.1.5923.1.1.1.1', 'eduPersonAffiliation', 'memberOf']
        }
    ...
    def handle_callback(self, callback_data: Dict[str, Any]) -> AuthenticationResult:
        ...
        try:
            ...
            return AuthenticationResult(
                ...
                institution=user_info.get('organization', user_info.get('institution', '')),
...
class InstitutionalSSOFactory:
    ...
    @classmethod
    def create_provider(cls, institution: str, custom_config: Dict[str, Any] = None) -> SSOProvider:
