...
class AuthMiddleware:
    ...
    def authenticate_cli(self, api_key: str = None, institution: str = None, user_id: str = None) -> UserProfile:
        """
    ...
    def authenticate_web(self, request_headers: Dict[str, str], cookies: Dict[str, str] = None) -> UserProfile:
        """
    ...
    def create_jwt_token(self, user_profile: UserProfile, expires_in: int = 3600) -> str:
    ...
    def require_auth(self, required_roles: list = None, required_permissions: list = None):
        """
        ...
        def decorator(func: Callable):
            ...
            def wrapper(*args, **kwargs):
                ...
                if required_roles:
                    user_role_values = [role.value for role in user_profile.institutional_roles]
        ...
        def decorator(func: Callable):
            ...
            def wrapper(*args, **kwargs):
                ...
                if institutions and user_profile.institution not in institutions:
                    raise AuthorizationError(f"Institution not allowed. Allowed: {institutions}")
                    
                # Ensure user has institutional context (not just API key)
                if user_profile.institution == "external":
                    raise AuthorizationError("Institutional authentication required (API key not sufficient)")
    ...
    def get_user_context(self, api_key: str = None, request_headers: Dict[str, str] = None) -> Optional[Dict[str, Any]]:
        """
    ...
    def log_user_activity(self, user_profile: UserProfile, action: str, resource: str = None, metadata: Dict[str, Any] = None):
...
def require_collection_permissions(func: Callable):
    ...
    auth = AuthMiddleware()
    return auth.require_auth(required_permissions=['create_collection', 'modify_metadata'])(func)
