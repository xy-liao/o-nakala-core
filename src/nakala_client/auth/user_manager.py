...
class UserManager:
    ...
    def create_user(
        ...
        self.log_activity(
            user_id=user_id,
            action="user_created",
            metadata={'institution': institution, 'roles': [role.value for role in user_profile.institutional_roles]}
        )
    ...
    def list_users(
        ...
        for user_data in users.values():
            ...
            if role:
                user_roles = [InstitutionalRole(r) for r in user_data.get('institutional_roles', [])]
