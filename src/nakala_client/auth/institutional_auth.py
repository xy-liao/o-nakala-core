...
class UserProfile:
    ...
    def can_manage_collections(self) -> bool:
        ...
        return any(role in self.institutional_roles for role in [
...
class InstitutionalPolicy:
    ...
    default_roles: List[InstitutionalRole] = field(default_factory=lambda: [InstitutionalRole.GUEST])
        ...
        return {
            'efeo': cls(
                ...
                role_mapping={
                    ...
                    'professor': [InstitutionalRole.PROFESSOR, InstitutionalRole.RESEARCHER],
                    'librarian': [InstitutionalRole.LIBRARIAN],
                    'admin': [InstitutionalRole.ADMIN],
                    'student': [InstitutionalRole.STUDENT]
                },
                max_upload_size=50 * 1024 * 1024 * 1024,  # 50GB for research institution
                allowed_file_types=['pdf', 'doc', 'docx', 'txt', 'jpg', 'png', 'tiff', 'xml', 'csv']
            ),
            'bnf': cls(
                institution='bnf',
                require_2fa=True,
                ...
                role_mapping={
                    'librarian': [InstitutionalRole.LIBRARIAN],
                    'archivist': [InstitutionalRole.ARCHIVIST, InstitutionalRole.LIBRARIAN],
            ...
            'unistra': cls(
                ...
                allowed_file_types=['pdf', 'doc', 'docx', 'txt', 'jpg', 'png', 'csv', 'xlsx', 'ppt', 'pptx']
            ),
            'huma_num': cls(
                ...
                max_upload_size=200 * 1024 * 1024 * 1024,  # 200GB for research consortium
                allowed_file_types=['*']
            )
        }
...
class InstitutionalAuthManager:
    ...
    def __init__(self, config_path: str = None):
        self.config_path = config_path or os.path.expanduser('~/.nakala/institutional_auth.json')
    ...
    def _initialize_providers(self):
        ...
        for institution in self.policies.keys():
            try:
                ...
                logger.warning(f"Failed to initialize SSO provider for {institution}: {e}")
                
    def authenticate_user(self, institution: str, callback_data: Dict[str, Any]) -> AuthenticationResult:
        ...
        if policy:
            ...
            if policy.require_institutional_email and policy.allowed_domains:
                email_domain = auth_result.email.split('@')[-1] if '@' in auth_result.email else ''
                if policy.allowed_domains != ['*'] and email_domain not in policy.allowed_domains:
                    return AuthenticationResult(
                        success=False,
                        user_id='',
                        email='',
        ...
        auth_result.attributes['institutional_roles'] = [role.value for role in user_profile.institutional_roles]
        auth_result.attributes['api_key'] = user_profile.api_key
        
        return auth_result
        
    def _create_or_update_user(self, auth_result: AuthenticationResult, institution: str) -> UserProfile:
        ...
        if policy:
            ...
            for sso_role in auth_result.roles:
                if sso_role in policy.role_mapping:
                    user_profile.institutional_roles.extend(policy.role_mapping[sso_role])
                    
            # Remove duplicates
            user_profile.institutional_roles = list(set(user_profile.institutional_roles))
        ...
        try:
            if os.path.exists(self.user_store_path):
                ...
                if user_data:
                    ...
                    institutional_roles = [
                        InstitutionalRole(role) for role in user_data.get('institutional_roles', [])
                    ]
    ...
    def _save_user_profile(self, user_profile: UserProfile):
        ...
        try:
            ...
            users_data[user_profile.user_id] = {
                ...
                'institutional_roles': [role.value for role in user_profile.institutional_roles],
    ...
    def validate_user_permissions(self, user_profile: UserProfile, action: str, resource: str = None) -> bool:
    ...
    def add_institution(self, institution: str, policy: InstitutionalPolicy, sso_config: Dict[str, Any] = None):
        """Add new institutional configuration."""
        self.policies[institution] = policy
        
        if sso_config:
            try:
                provider = InstitutionalSSOFactory.create_provider(institution, sso_config)
                self.providers[institution] = provider
            except Exception as e:
    ...
    def get_auth_url(self, institution: str, redirect_uri: str, state: str = None) -> Optional[str]:
