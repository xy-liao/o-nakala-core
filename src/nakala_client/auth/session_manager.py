...
class UserSession:
    ...
    def is_valid(self) -> bool:
        ...
        return (
            ...
            self.last_accessed > (datetime.now() - timedelta(hours=24))
        )
...
class SessionManager:
    ...
    def __init__(self, session_store_path: str = None):
        self.session_store_path = session_store_path or os.path.expanduser('~/.nakala/sessions.json')
    ...
    def refresh_session(self, session_id: str, extend_by: int = None) -> Optional[UserSession]:
        """Refresh session expiration."""
    ...
    def _cleanup_expired_sessions(self):
        ...
        if len(active_sessions) != len(sessions):
            ...
            logger.info(f"Cleaned up {len(sessions) - len(active_sessions)} expired sessions")
    ...
    def get_session_stats(self) -> Dict[str, Any]:
        ...
        for session in active_sessions:
            institution_counts[session.institution] = institution_counts.get(session.institution, 0) + 1
        ...
        return {
            ...
            'oldest_session': min(active_sessions, key=lambda s: s.created_at).created_at.isoformat() if active_sessions else None,
            'newest_session': max(active_sessions, key=lambda s: s.created_at).created_at.isoformat() if active_sessions else None
        }